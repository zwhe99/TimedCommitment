...
* 创建输入脚本，修改交易
  ...
  **注意：** 对于 *RelativeTimelockSolver* 中的第一个参数 *Sequence($x$)*(或*TimeBasedSequence($x$)* / *HeightBasedSequence($x$)* )， 应至少使 $x$ 大于等于 [*Commit* 交易](#*Commit* 交易) 中设置的sequence参数的值(上文中设置为5)，这里是解除 *时间锁定* 的地方。当然，一般设置为等于即可。
