...
* 开启阶段
  ...
  承诺方公开秘密$x$，完成承诺。每个诚实的接受者都可以确信，无论恶意承诺者的行为如何，承诺都可以以一种方式公开，即承诺者既不能反悔也不能使用$x^{'} \not=x$开启承诺。
...
* 解决方法
  ...
  * 首先，假设在协议启动之前，分类账包含n个未赎回的标准交易$U_{1}^{C}、...、U_{n}^{C}$，这些事务可以用一个仅为承诺方 C 已知的私钥进行赎回，且每笔交易具有 d BTC。
...
* 实现二
  ...
  * 实现一将“定时”放在 $PayDeposit_{i}$ 以阻止其在指定时间之前被矿工挖掘。
  * 通过比特币脚本的 OP_CHECKSEQUENCEVERIFY 操作和交易输入的 nSequence 属性，我们可以将“定时”放在  $Commit_{i}$ 的输出脚本中实现，以实现**相对**时间的定时承诺。如此一来避免了多方签名和链下传输交易，因为承诺方本来就要对 $Commit_{i}$ 进行签名的。(**绝对**时间的定时承诺应使用 OP_CHECKLOCKTIMEVERIFY 和 交易的 Locktime 属性)
...
* chainside-btcpy
  ...
  * 简介
    ...
    * btcpy 是完全离线工作的，不需要访问区块链。我们主要使用 btcpy 完成：脚本和交易的创建
 ...
 * 工具类网站
   ...
   * 使用 BlockCypher 的API生成地址
   ...
   pubk_hex = '03cfecd654690aaa8ab5ad5fb15818f44a3f341290f2ebf5e1c61ada78f8162844' # 公钥(十六进制)
   pubk = PublicKey.unhexlify(pubk_hex)
   address = P2pkhAddress(pubk.hash())
   
   privk_hex = 'd9e9d07e538cd27e4bfaa712c1e7893149670c10d27fc61471b11c7a4c918169' # 私钥(十六进制)
...
给定Raw Transaction 将其解码为语义化的 key-value 格式的交易。
...
输出：
   ...
   于是，我们将刚刚四笔交易的钱，通过一笔交易汇合在了一起。这样在下次创建交易时，就只需要一个输入交易了。
...
* nSequence (或称Sequence)
  ...
  * 在 btcpy 中关于 Sequence 有三个类，分别是：Sequence、 TimebasedSequence 和 HeightBasedSequence。

    其中 TimebasedSequence 和 HeightBasedSequence 是 Sequence的子类。

    * Sequence类最具有通用性，与[BIP 68](https://github.com/bitcoin/bips/blob/master/bip-0068.mediawiki#Deployment)中描述的完全一致，可以用任何 0 <= seq <= 0xffffffff 的数字初始化。

    * TimebasedSequence 需要用 比特(1<<22) 为 1 的数字初始化， 如果误用比特(1<<22) 为 0 的数字初始化会提示错误：

      ```python
      from btcpy.structs.transaction import TimeBasedSequence
      
      sequence = 0x00000005
      TimeBasedSequence(sequence)
      ```

      ```bash
      输出：
      ValueError: Trying to initialise TimeBasedSequence with number expressing block height
      ```

    * HeightBasedSequence 需要用 比特(1<<22) 为 0 的数字初始化， 如果误用比特(1<<22) 为 1 的数字初始化会提示错误：

      ```python
      from btcpy.structs.transaction import HeightBasedSequence
      
      sequence = 0x00400005
      HeightBasedSequence(sequence)
      ```

      ```
      输出：
      ValueError: Trying to initialise TimeBasedSequence with number expressing time
      ```

      注：这里的输出应是 ValueError: Trying to initialise HeightBasedSequence with number expressing time, btcpy包的作者写错了。
...
* 创建输出脚本
  ...
  * 定时承诺输出脚本
    ...
    下面给出了 HeightBased 和 TimedBased 的输出脚本的创建，本教程中我们以 HeightBased 的输出脚本为例。
 ...
 * [广播交易](#广播交易)
   ...
   可以看到该交易有一个输入，两个输出。第一个输出是**非标准**的脚本即定时承诺的脚本，第二个输出是标准的 pay-to-pubkey-hash 脚本。
...
* 准备好秘密的*原像*， 创建输入脚本和输出脚本
  ...
  # 创建输入脚本
  secret = 'I have an apple'  # 秘密的原像
  p2pkh_solver = P2pkhSolver(privk) # privk为承诺方的私钥
  hasklock_solver = HashlockSolver(secret.encode(), p2pkh_solver)
  if_solver = IfElseSolver(Branch.IF,  # branch selection
                           hasklock_solver)
  # 创建输出脚本
  script = P2pkhScript(pubk) # pubk为承诺方的公钥
  ```
...
* 准备好上一笔交易，获取押金的数额
  ...
  to_spend_raw = get_raw_tx(to_spend_hash, coin_symbol) # get raw transaction
  to_spend = TransactionFactory.unhexlify(to_spend_raw) # decode raw transaction
  penalty = int(float(to_spend.to_json()['vout'][0]['value']) * (10**8)) # get penalty
  ```
...
* 创建输入脚本，修改交易
  ...
  下面给出了 HeightBased 和 TimedBased 的输入脚本的创建，本教程中我们以 HeightBased 的输入脚本为例。
  ...
  else_solver = IfElseSolver(Branch.ELSE,  # Branch selection
                             RelativeTimelockSolver(HeightBasedSequence(0x00000002),
                                                    P2pkhSolver(privk)))# privk为 Recipient 的私钥
  ...
  **注意：** 对于 *RelativeTimelockSolver* 中的第一个参数 *Sequence($x$)*(或*TimeBasedSequence($x$)* / *HeightBasedSequence($x$)* )， 应至少使 $x$ 大于等于 [*Commit* 交易](#*Commit* 交易) 中设置的sequence参数的值，这里是解除 *时间锁定* 的地方。当然，一般设置为等于即可。
